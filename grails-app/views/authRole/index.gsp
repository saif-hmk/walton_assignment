...
<head>
    ...
    <g:set var="entityName" value="${message(code: 'label.authRole.pageTitle', default: 'Role')}"/>
        ...
        <div class="box box-primary">%{--Table Content--}%
            <div class="box-body">
                <table class="table dt-default table-bordered table-striped table-hover table-condensed display nowrap th-center th-colored">
                    <thead>
                    <tr>
                        <th>${message(code: 'label.authRole.rank', default: 'Rank')}</th>
                        <th>${message(code: 'label.authRole.authority', default: 'Authority')}</th>
                        <th>${message(code: 'label.authRole.remarks', default: 'Remarks')}</th>
                        <th>${message(code: 'label.authRole.isActive', default: 'Is Active')}</th>
                    ...
                    <g:if test="${authRoleInstanceList}">
                        <g:each in="${authRoleInstanceList}" status="i" var="authRoleInstance">
                            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                                <td class="left">${fieldValue(bean: authRoleInstance, field: "rank")}</td>
                                <td class="left">${fieldValue(bean: authRoleInstance, field: "authority")}</td>
                                <td class="left">${fieldValue(bean: authRoleInstance, field: "remarks")}</td>
                                <td class="center ${authRoleInstance.isActive ? 'c-success' : 'c-gray-light'}"><g:formatBoolean boolean="${authRoleInstance.isActive}" true="&#10004;" false="&#10008;"/></td>
                                <td class="center">
                                    ...
                                    <sec:access url="/authRole/delete/${authRoleInstance?.id}">
                                        <g:link class="btn btn-xs btn-danger confirm-delete" action="delete" id="${authRoleInstance?.id}">%{-- onclick="return confirm('Are you sure you want to delete this record?');">--}%
                    ...
                    %{--<g:else>--}%
                        %{--<tr><td class="bold center" colspan="4"><g:message code="default.message.noRecordFound" default="No Record Found...!"/></td></tr>--}%
                    %{--</g:else>--}%
                    </tbody>
                </table>
                ...
                %{--
                <div class="paginate">
                    <g:paginate total="${org.aalaa.auth.AuthRole?.count()}"/>
                </div>
                --}%
