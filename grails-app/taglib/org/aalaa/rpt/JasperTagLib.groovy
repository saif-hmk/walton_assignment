...
class JasperTagLib {
     ...
     * @attr title - OPTIONAL title of the report
    ...
    def jasperReport = { GroovyPageAttributes attrs, body ->
        ...
        if (bodyContent) {
            ...
            out << """<form class="${formClass}"${idAttr ? ' id="' + idAttr + '"' : ''} title="${jasperName}" action="${g.createLink(controller: controller, action: action)}">"""
            out << """<input type="hidden" title="_format" />
<input type="hidden" title="_name" value="${reportName}" />
<input type="hidden" title="_file" value="${jasperName}" />
"""

            if (attrs['inline']) {
                out << """<input type="hidden" title="_inline" value="${attrs['inline']}" />\n"""
            }
    ...
    protected String renderJavascriptForForm(String jasperNameNoPunct) {
        // function gets a unique title in case this tag is repeated on the page
        """
...
//Beginning of the new Tags added at plugin version 0.9.5
    ...
    def jasperForm = { GroovyPageAttributes attrs, body ->
        ...
        out << """
            <form class="${formClass}" title="${jasperName}" action="${g.createLink(controller: controller, action: action, id: id)}">
            <input type="hidden" title="_format" />
            <input type="hidden" title="_name" value="${reportName}" />
            <input type="hidden" title="_file" value="${jasperName}" />
        """
     ...
     * Use the jasperButton-Tag inside a jasperForm to submit and generate the report.
     * 
     * @attr format - REQUIRED The title of the supported output format. ex. 'pdf' or 'PDF'.
    ...
    def jasperButton = { GroovyPageAttributes attrs ->
        if (!attrs['format']) {
            throw new Exception(message(code: "jasper.taglib.missingAttribute", args: 'format'))
        }
